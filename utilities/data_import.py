...
def sentences_from_margot_json(file_name):
    ...
    for sentence in data["document"]:
        if (sentence["claim_score"] > 0 or sentence["evidence_score"] > 0) and len(
            sentence["text"]
        ) > 7:
            claims_or_evidences.append(
                (sentence["text"], sentence["claim_score"], sentence["evidence_score"])
            )
    f.close()
...
def sentences_from_ECHR_corpus(file_name):
    ...
    for document in data:
        for clause in document["clauses"]:
            sentences[clause["_id"]] = [
                document["text"][clause["start"] : clause["end"]],
                False,
            ]
